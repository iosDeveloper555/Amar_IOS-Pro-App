//
//  JobsClosedViewController.swift
//  PlumberJJ
//
...
class JobsClosedViewController: RootBaseViewController, UITableViewDataSource,UITableViewDelegate {
    @IBOutlet var tableViewFooter: MyFooter!
    @IBOutlet weak var jobsClosedTbl: UITableView!
    var jobsClosedArr:NSMutableArray=NSMutableArray()
 ...
 //   var theme:Theme=Theme()
    ...
    override func viewDidLoad() {
        ...
        jobsClosedTbl.register(UINib(nibName: "newleadsxibTableViewCell", bundle: nil), forCellReuseIdentifier: "cell")
       NotificationCenter.default.addObserver(self, selector: #selector(JobsClosedViewController.methodOfReceivedSortingNotificationNetworkDetail(_:)), name:NSNotification.Name(rawValue: "SortingJobNotification"), object: nil)
        
        
        jobsClosedTbl.isHidden=true
        nextPageStr=1
        refreshNewLeads()
        showProgress()
    ...
    @objc func methodOfReceivedSortingNotificationNetworkDetail(_ notification: Notification){
        ...
        Param  = ["provider_id":"\(objUserRecs.providerId)",
                  "page":"\(nextPageStr)",
                   "type":"4","perPage":kPageCount,"from":userInfo["Fromdate"]!,"to":userInfo["Todate"]!,"orderby":userInfo["asDes"]!,"sortby":userInfo["StatusforSort"]!]
    ...
    func GetNewLeads(){
        ...
        url_handler.makeCall(myJobsUrl , param: Param as NSDictionary) {
            ...
            self.jobsClosedTbl.isHidden=false
            self.DismissProgress()
            self.jobsClosedTbl.dg_stopLoading()
            self.loading = false
            if(error != nil)
            {
                ...
                if(responseObject != nil && (responseObject?.count)!>0)
                {
                    ...
                    if(status == "1")
                    {
                        if(self.nextPageStr==1){
                            self.jobsClosedArr.removeAllObjects()
                        }
...
//                                self.view.makeToast(message:self.theme.setLang("no_leads"), duration: 3, position: HRToastPositionDefault as AnyObject, title:appNameJJ)
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        if(jobsClosedArr.count>0){
            ...
            if(noDataView==nil){
                ...
                noDataView.msgLbl.text = "You do not have any completed leads"
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let objRec:MyOrderOpenRecord=self.jobsClosedArr.object(at: indexPath.row) as! MyOrderOpenRecord
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(!loading){
            if(indexPath.row < jobsClosedArr.count){
                if didselect == ""
                {
                    let objRec=jobsClosedArr.object(at: indexPath.row)
    ...
    func refreshNewLeads(){
        ...
        jobsClosedTbl.dg_addPullToRefreshWithActionHandler({
            self.nextPageStr=1
        ...
        jobsClosedTbl.dg_setPullToRefreshFillColor(PlumberLightGrayColor)
        jobsClosedTbl.dg_setPullToRefreshBackgroundColor(jobsClosedTbl.backgroundColor!)
    }
    ...
    deinit {
        jobsClosedTbl.dg_removePullToRefresh()
