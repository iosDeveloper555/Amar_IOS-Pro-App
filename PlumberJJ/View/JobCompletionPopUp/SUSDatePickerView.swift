...
class SUSDatePickerView: CSAnimationView {
    ...
    convenience init(frame:CGRect, delegate:DatePickerDelegate, mode:UIDatePicker.Mode, button:UIButton) {
    ...
    func addDatePicker(_ mode:UIDatePicker.Mode) {
        ...
        dateLbl.text = theme.setLang("date")
        ...
        datePicker.addTarget(self, action: #selector(SUSDatePickerView.didChange(_:)), for: UIControl.Event.valueChanged)
        ...
        backBTn.setTitle(theme.setLang("Back"), for: UIControl.State())
        backBTn.addTarget(self, action:#selector(SUSDatePickerView.cancelButtonClicked(_:)), for: UIControl.Event.touchUpInside)
        ...
        setBTn.setTitle(theme.setLang("Set"), for: UIControl.State())
        setBTn.addTarget(self, action:#selector(SUSDatePickerView.doneButtonClicked(_:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func didChange(_ sender:UIDatePicker){
        ...
        if self.datePickerMode == UIDatePicker.Mode.date {
            dateformater.dateFormat = "dd-MM-yyyy"
        } else if self.datePickerMode == UIDatePicker.Mode.time {
    ...
    @objc func cancelButtonClicked(_ sender:UIButton) {
        theme.MakeAnimation(view: self, animation_type: CSAnimationTypePopAlphaOut)
